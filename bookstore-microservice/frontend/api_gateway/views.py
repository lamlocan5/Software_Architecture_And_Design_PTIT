...
def _log_behavior(user_id, product_id, behavior_type="view"):
    if not user_id or not product_id:
        return
    try:
        requests.post(
            f"{RECOMMENDER_SERVICE_URL}/behavior/",
            json={"user_id": int(user_id), "product_id": int(product_id), "behavior_type": behavior_type},
            timeout=0.5
        )
...
def home(request):
    ...
    if _can_manage_books(request.user) or _is_admin(request.user):
        ...
        # Live health check of microservices backend concurrently
        from concurrent.futures import ThreadPoolExecutor
        services_to_check = [
            {"id": "user", "name": "User Service", "port": ":8001", "url": "http://user-service:8000", "desc": "Tài khoản, khách hàng, nhân viên, quản lý."},
            {"id": "product", "name": "Product Service", "port": ":8002", "url": "http://product-service:8000", "desc": "Quản lý sách, quần áo, thiết bị điện tử."},
            {"id": "cart", "name": "Cart Service", "port": ":8003", "url": "http://cart-service:8000", "desc": "Xử lý giỏ hàng tạm thời của khách."},
            {"id": "order", "name": "Order Service", "port": ":8004", "url": "http://order-service:8000", "desc": "Tạo đơn hàng, kiểm soát thanh toán/vận chuyển."},
            {"id": "review", "name": "Review Service", "port": ":8005", "url": "http://review-service:8000", "desc": "Bình luận, chấm điểm đánh giá sản phẩm."},
            {"id": "shipping", "name": "Shipping Service", "port": ":8006", "url": "http://shipping-service:8000", "desc": "Xử lý giao nhận, điều phối nhà vận chuyển."},
            {"id": "payment", "name": "Payment Service", "port": ":8007", "url": "http://payment-service:8000", "desc": "Thanh toán hóa đơn qua ngân hàng giả lập."},
            {"id": "catalogue", "name": "Catalogue Service", "port": ":8008", "url": "http://catalogue-service:8000", "desc": "Tổng hợp, tra cứu dữ liệu sách nhanh chóng."},
            {"id": "notification", "name": "Notification Service", "port": ":8009", "url": "http://notification-service:8000", "desc": "Gửi email thông báo, đẩy tin nhắn thời gian thực."},
            {"id": "ai", "name": "AI Service", "port": ":8011", "url": "http://ai-service:8000", "desc": "Trợ lý ảo chatbot, gợi ý sản phẩm phù hợp."}
        ]

        def _check_service(item):
            try:
                requests.get(item["url"], timeout=0.3)
                item["status"] = "online"
            except Exception:
                item["status"] = "offline"
            return item
        ...
        with ThreadPoolExecutor(max_workers=len(services_to_check)) as executor:
            results = list(executor.map(_check_service, services_to_check))
        ...
        return render(request, "index.html", {
            ...
            "service_count":   len(services_to_check),
            "services_online_count": online_count,
    ...
    if customer_id:
        ...
        for item in (items if isinstance(items, list) else []):
            ...
            if item.get("book_id") and item.get("book_id") != 0:
                prod_id = item.get("book_id")
            elif item.get("clothing_id"):
                prod_id = item.get("clothing_id")
            elif item.get("electronic_id"):
                prod_id = item.get("electronic_id")
        ...
        # Fetch hybrid recommendations from AI service
        try:
            reco_res = _get(f"{RECOMMENDER_SERVICE_URL}/recommend/?user_id={customer_id}&limit=4", {})
            if isinstance(reco_res, dict) and "recommended_ids" in reco_res:
                reco_ids = reco_res.get("recommended_ids", [])
                recommended = [product_map.get(rid) for rid in reco_ids if rid in product_map]
                recommended = [b for b in recommended if b]
        except Exception:
            recommended = []
...
def clothe_detail(request, clothe_id):
    ...
    if request.method == "POST":
        ...
        if not cart_id:
            ...
            try:
                ...
                if r.status_code == 200:
                    _log_behavior(customer_id, clothe_id, "cart")
        ...
        _log_behavior(customer_id, clothe_id, "view")
...
def electronics_list(request):
        ...
        try:
            ...
            try:
                if action == "update":
                    ...
                    r = _patch(f"{ELECTRONIC_SERVICE_URL}/electronics/{eid}/", payload)
                elif action == "delete":
                    eid = request.POST.get("id")
                    r = requests.delete(f"{ELECTRONIC_SERVICE_URL}/electronics/{eid}/", timeout=3)
                else:
                    r = _post(f"{ELECTRONIC_SERVICE_URL}/electronics/", payload)

                if r.status_code in (200, 201, 204):
                    return redirect("electronics_list")
                message = {"type": "danger", "text": f"Thao tác với đồ điện tử thất bại: {getattr(r, 'text', '')}"}
            except Exception as e:
                message = {"type": "danger", "text": f"Lỗi kết nối electronic-service: {e}"}
    ...
    categories = _get(f"{ELECTRONIC_SERVICE_URL}/electronics/categories/", [])
    if not isinstance(categories, list):
        categories = []
    ...
    if request.method == "POST":
        ...
        if not cart_id:
            ...
            try:
                ...
                if r.status_code == 200:
                    _log_behavior(customer_id, electronic_id, "cart")
                    return redirect("cart_view", customer_id=customer_id)
                message = {"type": "danger", "text": f"Thêm vào giỏ thất bại: {getattr(r, 'text', '')}"}
            except Exception as e:
                message = {"type": "danger", "text": f"Lỗi kết nối cart-service: {e}"}
    else:
        _log_behavior(customer_id, electronic_id, "view")
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not cart_id:
                ...
                if r.status_code == 200:
                    _log_behavior(customer_id, book_id, "cart")
                    return redirect("cart_view", customer_id=customer_id)
                message = {"type": "danger", "text": f"Thêm vào giỏ thất bại: {r.text}"}
        except Exception as e:
            message = {"type": "danger", "text": f"Lỗi kết nối cart-service: {e}"}
    else:
        _log_behavior(customer_id, book_id, "view")
...
def staff_list(request):
    ...
    if request.method == "POST":
        ...
        if action == "toggle_active":
            ...
            try:
                ...
                message = {"type": "danger", "text": f"Lỗi cập nhật staff-service: {e}"}

        elif action == "delete":
            staff_id = request.POST.get("staff_id")
            try:
                r = requests.delete(f"{STAFF_SERVICE_URL}/staff/{int(staff_id)}/", timeout=3)
                if r.status_code in (200, 204):
                    return redirect("staff_list")
                message = {"type": "danger", "text": "Xoá staff thất bại!"}
            except Exception as e:
            ...
            if not staff_id or not name or not email:
                ...
                try:
                    ...
                    r = _patch(f"{STAFF_SERVICE_URL}/staff/{int(staff_id)}/", payload)
            ...
            if not name or not email or len(password) < 6:
                ...
                try:
                    r = _post(f"{STAFF_SERVICE_URL}/staff/", {"name": name, "email": email, "password": password})
    ...
    for s in staff:
        s["login_active"] = s.get("active", True)
...
def manager_list(request):
    ...
    if request.method == "POST":
        ...
        if action == "toggle_active":
            ...
            try:
                ...
                message = {"type": "danger", "text": f"Lỗi cập nhật manager-service: {e}"}

        elif action == "delete":
            manager_id = request.POST.get("manager_id")
            try:
                r = requests.delete(f"{MANAGER_SERVICE_URL}/managers/{int(manager_id)}/", timeout=3)
                if r.status_code in (200, 204):
                    return redirect("manager_list")
                message = {"type": "danger", "text": "Xoá manager thất bại!"}
            except Exception as e:
            ...
            if not manager_id or not name or not email:
                message = {"type": "danger", "text": "Thiếu thông tin (id/name/email)."}
            else:
                try:
                    payload = {"name": name, "email": email}
                    if new_password:
                        if len(new_password) < 6:
                            raise ValueError("Mật khẩu mới phải >= 6 ký tự.")
                        payload["password"] = new_password
                    r = _patch(f"{MANAGER_SERVICE_URL}/managers/{int(manager_id)}/", payload)
                    if getattr(r, "status_code", 200) >= 400:
            ...
            if not name or not email or len(password) < 6:
                ...
                try:
                    r = _post(f"{MANAGER_SERVICE_URL}/managers/", {"name": name, "email": email, "password": password})
                    if r.status_code in (200, 201):
                        return redirect("manager_list")
    ...
    for m in managers:
        m["login_active"] = m.get("active", True)
...
def notification_list(request):
    ...
    # In docker, we can access the notification service by its service name
    NOTIFICATION_SERVICE_URL = "http://notification-service:8000"
    if is_admin:
        url = f"{NOTIFICATION_SERVICE_URL}/notifications/"
    else:
        url = f"{NOTIFICATION_SERVICE_URL}/notifications/customer/{customer_id}/"
